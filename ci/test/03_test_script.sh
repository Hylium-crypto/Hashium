...
if [ "$RUN_TIDY" != "true" ]; then
  ...
  # See https://github.com/hashium/hashium/pull/28359#issuecomment-1698694748
...
if [ "$RUN_FUZZ_TESTS" = "true" ]; then
  ...
  if [ ! -d "$DIR_FUZZ_IN" ]; then
    ${CI_RETRY_EXE} git clone --depth=1 https://github.com/hashium-core/qa-assets "${DIR_QA_ASSETS}"
  fi
  (
    cd "${DIR_QA_ASSETS}"
  ...
  if [ ! -d "$DIR_UNIT_TEST_DATA" ]; then
    ...
    ${CI_RETRY_EXE} curl --location --fail https://github.com/hashium-core/qa-assets/raw/main/unit_test_data/script_assets_test.json -o "${DIR_UNIT_TEST_DATA}/script_assets_test.json"
  fi
fi

# Make sure default datadir does not exist and is never read by creating a dummy file
if [ "$CI_OS_NAME" == "macos" ]; then
  echo > "${HOME}/Library/Application Support/Hashium"
else
  echo > "${HOME}/.hashium"
fi
...
HSMIUM_CONFIG_ALL="-DBUILD_BENCH=ON -DBUILD_FUZZ_BINARY=ON"
if [ -z "$NO_DEPENDS" ]; then
  HSMIUM_CONFIG_ALL="${HSMIUM_CONFIG_ALL} -DCMAKE_TOOLCHAIN_FILE=$DEPENDS_DIR/$HOST/toolchain.cmake"
fi
if [ -z "$NO_WERROR" ]; then
  HSMIUM_CONFIG_ALL="${HSMIUM_CONFIG_ALL} -DWERROR=ON"
fi
...
HSMIUM_CONFIG_ALL="$HSMIUM_CONFIG_ALL -DCMAKE_INSTALL_PREFIX=$BASE_OUTDIR -Werror=dev"

if [[ "${RUN_TIDY}" == "true" ]]; then
  HSMIUM_CONFIG_ALL="$HSMIUM_CONFIG_ALL -DCMAKE_EXPORT_COMPILE_COMMANDS=ON"
fi

eval "CMAKE_ARGS=($HSMIUM_CONFIG_ALL $HSMIUM_CONFIG)"
...
if [ "${RUN_TIDY}" = "true" ]; then
  cmake -B /tidy-build -DLLVM_DIR=/usr/lib/llvm-"${TIDY_LLVM_V}"/cmake -DCMAKE_BUILD_TYPE=Release -S "${BASE_ROOT_DIR}"/contrib/devtools/hashium-tidy
  cmake --build /tidy-build "$MAKEJOBS"
  cmake --build /tidy-build --target hashium-tidy-tests "$MAKEJOBS"
  ...
  if ! ( run-clang-tidy-"${TIDY_LLVM_V}" -quiet -load="/tidy-build/libhashium-tidy.so" "${MAKEJOBS}" | tee tmp.tidy-out.txt ); then
  ...
  run_iwyu() {
    ...
    python3 "/include-what-you-use/iwyu_tool.py" \
             ...
             -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_ROOT_DIR}/contrib/devtools/iwyu/hashium.core.imp" \
