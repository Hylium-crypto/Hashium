...
be used in Hashium Core with the `-asmap` option.
...
to binary before being used with Hashium Core.
...
the `hashium-cli getnodeaddresses` command.
...
```
hashium-cli getnodeaddresses 0 > addrs.json
```
