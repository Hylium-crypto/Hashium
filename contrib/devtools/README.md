...
Provides utilities for managing copyright headers of `The Hashium Core
...
Generates a hashium.conf file in `share/examples/` by parsing the output from `hashiumd --help`. This script is run during the
release process to include a hashium.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-hashium-conf.sh
```
