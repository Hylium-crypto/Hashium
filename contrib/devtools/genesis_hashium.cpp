// Genesis finder for Hashium networks (main/test/test4/regtest).
...
//
// Compile:
//   c++ -O3 -std=c++17 contrib/devtools/genesis_hashium.cpp -lcrypto -o genesis_hashium
//
// Run:
//   ./genesis_hashium
//
...
// minutes of CPU time. Use more threads by setting HSMIUM_GENESIS_THREADS=N.
...
{
    ByteVec script_pubkey = MakeOpReturn("Hashium genesis");
    auto target = BitsToTarget(spec.nBits);
    bool use_extra_nonce = spec.use_extra_nonce;
    uint32_t extra_start = spec.extra_nonce;
    if (const char* v = std::getenv("HSMIUM_GENESIS_EXTRANONCE")) {
        use_extra_nonce = std::strtoul(v, nullptr, 0) != 0;
    }
    if (const char* v = std::getenv("HSMIUM_GENESIS_EXTRANONCE_START")) {
        extra_start = static_cast<uint32_t>(std::strtoul(v, nullptr, 0));
    }
    const uint32_t nonce_start = GetEnvUint("HSMIUM_GENESIS_START", 0);
    ...
    for (uint32_t extra_nonce = use_extra_nonce ? extra_start : spec.extra_nonce;; ++extra_nonce) {
        ...
        if (!use_extra_nonce) {
            std::cout << "\nNo solution found for fixed coinbase; set HSMIUM_GENESIS_EXTRANONCE=1 to expand search.\n";
            return;
        }
...
int main()
{
    ...
    if (const char* env = std::getenv("HSMIUM_GENESIS_THREADS")) {
    ...
    std::vector<NetSpec> nets = {
        {"main",   "Hashium 2025-12-16: Verify, don't trust.", MAIN_TIME, BITS_STD, true, 0},
        {"testnet","Hashium testnet 2025-12-16: break safely.", MAIN_TIME, BITS_EZ, false, 0},
        {"test4",  "Hashium test4 2025-12-16: staging chain.", MAIN_TIME, BITS_EZ, false, 0},
        {"regtest","Hashium regtest: local sandbox.", 1296688602, BITS_EZ, false, 0},
    };

    // Optional filter: comma-separated list in HSMIUM_GENESIS_NETS
    std::vector<std::string> filter;
    if (const char* env = std::getenv("HSMIUM_GENESIS_NETS")) {
