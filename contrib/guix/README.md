# Bootstrappable Hashium Core Builds

This directory contains the files necessary to perform bootstrappable Hashium
Core builds.
...
To build Hashium Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Hashium Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/hashium-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/hashium-core/guix.sigs).
