#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Hashium Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/hashium-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Hashium Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Hashium Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example hashium.conf file. if contrib/devtools/gen-hashium-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/hashium.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/hashium-macos-app.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
