...
"""Script for verifying Hashium Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from hashiumcore.org and hashium.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/hashium-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://hashiumcore.org"
HOST2 = "https://hashium.org"
VERSIONPREFIX = "hashium-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Hashium community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"hashium_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by hashiumcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the hashium release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes hashium.org lags behind hashiumcore.org.)')
    )
