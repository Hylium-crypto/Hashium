...
rpcA(){ hashium-cli -regtest -datadir="$A" -rpcport="$A_RPC" -rpcuser=auser -rpcpassword=apass "$@"; }
rpcB(){ hashium-cli -regtest -datadir="$B" -rpcport="$B_RPC" -rpcuser=buser -rpcpassword=bpass "$@"; }

cleanup() {
  rpcA stop 2>/dev/null || true
  rpcB stop 2>/dev/null || true
  pkill -f "hashiumd.*datadir=$A" 2>/dev/null || true
  pkill -f "hashiumd.*datadir=$B" 2>/dev/null || true
}
trap cleanup EXIT
...
pkill -f hashiumd 2>/dev/null || true
sleep 1
...
echo "[*] start daemons"
hashiumd -datadir="$A" -daemon
hashiumd -datadir="$B" -daemon
