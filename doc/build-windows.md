...
Below are some notes on how to build Hashium Core for Windows.

The options known to work for building Hashium Core on Windows are:
...
Note that for WSL the Hashium Core source path MUST be somewhere in the default mount file system, for
example /usr/src/hashium, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\hashium`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/hashium
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/hashium --strip
```
