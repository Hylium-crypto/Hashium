# CJDNS support in Hashium Core

It is possible to run Hashium Core over CJDNS, an encrypted IPv6 network that
...
done outside of Hashium Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Hashium network and individual nodes.
...
before it will work with your Hashium Core node. This is described in steps
...
## Run Hashium Core with CJDNS

Once you are connected to the CJDNS network, the following Hashium Core
...
When enabled, this option tells Hashium Core that it is running in an
...
IPv6 local network. This helps Hashium Core perform better address management:
...
CJDNS support was added to Hashium Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `hashium-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/hashium/hashium/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Hashium Core

There are several ways to see your CJDNS address in Hashium Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `hashium-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `hashium-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `hashium-cli help getnodeaddresses` for details.
