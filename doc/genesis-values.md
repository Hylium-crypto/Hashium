# Hashium Genesis Values (WIP)

Goal: derive final genesis parameters for all networks (main/testnet/test4/regtest) using `contrib/devtools/genesis_hashium.cpp`. Once hashes/nonces are found, copy them here and into `chainparams.cpp` later.

## Common parameters
- Block time: 600s
- Supply/halving: 21M cap, halving every 210,000 blocks
- Coinbase output: OP_RETURN "Hashium genesis" (provably unspendable, no premine)
...
- Messages:
  - main:    `Hashium 2025-12-16: Verify, don't trust.`
  - testnet: `Hashium testnet 2025-12-16: break safely.`
  - test4:   `Hashium test4 2025-12-16: staging chain.`
  - regtest: `Hashium regtest: local sandbox.`
...
c++ -O3 -march=native -funroll-loops -std=c++17 \
  ...
  contrib/devtools/genesis_hashium.cpp -lcrypto -o /tmp/genesis_hashium
```

Run (per network; uses all CPUs by default):
```sh
HSMIUM_GENESIS_THREADS=$(sysctl -n hw.ncpu) HSMIUM_GENESIS_NETS=main /tmp/genesis_hashium
HSMIUM_GENESIS_THREADS=$(sysctl -n hw.ncpu) HSMIUM_GENESIS_NETS=testnet /tmp/genesis_hashium
HSMIUM_GENESIS_THREADS=$(sysctl -n hw.ncpu) HSMIUM_GENESIS_NETS=test4 /tmp/genesis_hashium
HSMIUM_GENESIS_THREADS=$(sysctl -n hw.ncpu) HSMIUM_GENESIS_NETS=regtest /tmp/genesis_hashium
```

To resume from a higher nonce range (e.g. after abort), set:
```sh
HSMIUM_GENESIS_START=<nonce_offset>
```
Example: `HSMIUM_GENESIS_START=800000000`.
