# Hashium P2P Network Security

**Version:** 1.0
**Last Updated:** 2025-12-24

This document outlines the security measures implemented in the Hashium P2P network layer to protect against Denial of Service (DoS) attacks, resource exhaustion, and malicious peers.
...
The Hashium P2P network is a peer-to-peer overlay network where nodes communicate directly without a central authority. This architecture is robust but vulnerable to various attacks. Our security model follows these principles:
...
```bash
hashium-cli setban "192.168.1.1" add 86400  # Ban for 24 hours
hashium-cli setban "192.168.1.0/24" add      # Ban subnet indefinitely
```
...
3. **Mempool Health:**
   ```bash
   hashium-cli getmempoolinfo
   # Watch: size, bytes, usage
   ```

4. **Ban List:**
   ```bash
   hashium-cli listbanned
