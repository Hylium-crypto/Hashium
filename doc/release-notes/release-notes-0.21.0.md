...
Hashium Core version 0.21.0 is now available from:

  <https://hashiumcore.org/bin/hashium-core-0.21.0/>
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Hashium
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Hashium Core on
unsupported systems.

From Hashium Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Hashium Core does not yet change appearance
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  [BIP155](https://github.com/hashium/bips/blob/master/bip-0155.mediawiki).
  Version 2 hidden services are still fully supported by Hashium Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Hashium Core will no longer
...
- This release adds support for serving
  [BIP157](https://github.com/hashium/bips/blob/master/bip-0157.mediawiki) compact
...
- This release adds support for signets
  ([BIP325](https://github.com/hashium/bips/blob/master/bip-0325.mediawiki)) in
...
- This release implements
  [BIP339](https://github.com/hashium/bips/blob/master/bip-0339.mediawiki)
...
- This release implements the proposed Taproot consensus rules
  ([BIP341](https://github.com/hashium/bips/blob/master/bip-0341.mediawiki) and
  [BIP342](https://github.com/hashium/bips/blob/master/bip-0342.mediawiki)),
...
- A new `hashium-cli -netinfo` command provides a network peer connections
...
- A new `hashium-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `hashium-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `hashium-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Hashium Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `hashium-wallet` tool which performs the salvage
...
Hashium Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`hashium.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Hashium Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `hashium-cli createwallet` or `hashium-wallet
...
Bugs and missing functionality can be reported to the [issue tracker](https://github.com/hashium/hashium/issues).
...
Users who already have a Hashium Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Hashium Core.
...
The change to using descriptors changes the default derivation paths used by Hashium Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in HSM per 1,000
  vbytes (HSM/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from HSM/kB
  to sat/vB and the help documentation in HSM/kB is updated to HSM/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from HSM/kvB to sat/vB and the
  ...
  difference between HSM/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in HSM/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Hashium Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `hashium.conf`
...
- #19284 Add seed.hashium.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in hashium-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add hashium-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in hashium-cli (laanwj)
...
### Build system
- #18504 Drop hashium-tx and hashium-wallet dependencies on libevent (ryanofsky)
...
- #18297 Use pkg-config in HSMIUM_QT_CONFIGURE for all hosts including Windows (hebasto)
...
- #18653 add coverage for hashium-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Hashium Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitrehashium)
...
- #18817 Document differences in hashiumd and hashium-qt locale handling (practicalswift)
