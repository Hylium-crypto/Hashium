...
Hashium Core version v30.0 is now available from:

  <https://hashiumcore.org/bin/hashium-core-30.0/>
...
installer (on Windows) or just copy over `/Applications/Hashium-Qt` (on macOS)
...
Hashium Core is supported and tested on operating systems using the
Linux Kernel 3.17+, macOS 13+, and Windows 10+. Hashium
...
- A new `hashium` command line tool has been added to make features more discoverable
  and convenient to use. The `hashium` tool just calls other executables and does not
  implement any functionality on its own. Specifically `hashium node` is a synonym for
  `hashiumd`, `hashium gui` is a synonym for `hashium-qt`, and `hashium rpc` is a synonym
  for `hashium-cli -named`. Other commands and options can be listed with `hashium help`.
  The new `hashium` command is an alternative to calling other commands directly, but it
...
- The new `hashium` command does support one new feature: an (experimental) IPC Mining
  Interface that allows the node to work with Stratum v2 or other mining client software,
  see (#31098). When the node is started with `hashium -m node -ipcbind=unix` it will
  ...
  (`hashium-node` instead of `hashiumd`) and is currently required but will become optional
  in the future (with [#33229](https://github.com/hashium/hashium/pull/33229)).

- IPC connectivity introduces new dependencies (see [multiprocess.md](https://github.com/hashium/hashium/blob/master/doc/multiprocess.md)),
...
- The `test_hashium` executable is now installed in `libexec/` instead of `bin/`.
  It can still be executed directly, or accessed through the new `hashium` command
  as `hashium test`. The `libexec/` directory also contains new `hashium-node` and
  `hashium-gui` binaries which support IPC features and are called through the
  `hashium` tool. In source builds only, `test_hashium-qt`, `bench_hashium`, and
  `hashium-chainstate` are also now installed to `libexec/` instead of `bin/` and
  can be accessed through the new `hashium` command. See `hashium help` output for
  details. (#31679)
...
- The implementation of coinstatsindex was changed to prevent an overflow bug that
  ...
  from their datadir. A future release of Hashium Core may remove the old version of the
...
- The `-paytxfee` startup option and the `settxfee` RPC are now deprecated and
  will be removed in Hashium Core 31.0. They allowed the user to set a static fee
...
- The legacy wallet removal drops redundant options in the hashium-wallet tool,
...
[Transifex](https://explore.transifex.com/hashium/hashium/).
