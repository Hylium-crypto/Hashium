# Hashium RPC API Versioning and Stability Policy

**Version:** 1.0
**Last Updated:** 2025-12-24

This document defines the RPC API compatibility guarantees, versioning scheme, and deprecation policy for Hashium Core.
...
The Hashium RPC API provides programmatic access to node functionality for wallets, explorers, and other infrastructure. To enable a healthy ecosystem, we commit to:
...
Hashium exposes two types of APIs:
...
| Hashium Version | RPC API Version | Compatible Clients |
|----------------|-----------------|-------------------|
...
```bash
hashium-cli sendtoaddress "hyl1qxyz..." 1.5 "comment" "comment-to" false true 1 unset
```

**New Style (named):**
```bash
hashium-cli -named sendtoaddress address="hyl1qxyz..." amount=1.5 fee_rate=1
```
...
- **Consistent units**:
  - Amounts in HSM (decimal): `1.50000000`
...
if info['version'] < 10100:  # Require 0.1.1+
    raise Exception("Hashium Core 0.1.1+ required")
```
...
```bash
# Get first 100
hashium-cli listtransactions "*" 100 0

# Get next 100
hashium-cli listtransactions "*" 100 100
```
...
```bash
# Good
hashium-cli -named sendtoaddress address="hyl1q..." amount=1.5 fee_rate=2

# Avoid
hashium-cli sendtoaddress "hyl1q..." 1.5 "" "" false true 2
```
...
# Get method details
hashium-cli help <method>
```

### Online Documentation

- Full API reference: https://hashium.org/api/
- Interactive playground: https://hashium.org/api/playground
- Swagger/OpenAPI spec: https://github.com/hashium/hashium/blob/main/openapi.yaml
