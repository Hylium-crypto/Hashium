# Hashium Testing Strategy and Improvement Plan

**Version:** 1.0
**Last Updated:** 2025-12-24

This document outlines the comprehensive testing strategy for Hashium to ensure consensus correctness, prevent regressions, and maintain network stability.
...
✅ **Already Implemented:**
- `feature_hashium_devnet_smoke.py` - Basic node startup and block generation
- `feature_hashium_p2p_propagation.py` - Transaction/block propagation
- `feature_hashium_reorg_basic.py` - Simple reorganization scenarios
...
**Structure:**
```python
class MyTestCase(HashiumTestFramework):
...
jobs:
  unit-tests:
    ...
    steps:
      - name: Run unit tests
        run: |
          cmake --build build --target test_hashium
          ./build/src/test/test_hashium
...
./build/src/test/test_hashium --run_test=consensus_tests || exit 1
...
# Specific test
./test/functional/feature_hashium_reorg_basic.py
