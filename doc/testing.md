# Hashium Testing Guide

This guide explains how to build and test Hashium Core.
...
```bash
cd /Users/Apple/Desktop/Hashium
...
```bash
# All unit tests
./build/bin/test_hashium

# Specific test suite
./build/bin/test_hashium -t wallet_tests

# List available tests
./build/bin/test_hashium -l test_suite
```
...
### Run Hashium-Specific Tests

```bash
# Security checks
python3 test/functional/feature_hashium_security_check.py

# Devnet smoke test
python3 test/functional/feature_hashium_devnet_smoke.py

# P2P propagation
python3 test/functional/feature_hashium_p2p_propagation.py

# Reorg handling
python3 test/functional/feature_hashium_reorg_basic.py

# Wallet functionality
python3 test/functional/feature_hashium_wallet_basic.py

# Mining functionality
python3 test/functional/feature_hashium_mining_basic.py
```
...
# Trace all RPC calls
python3 test/functional/feature_hashium_security_check.py --tracerpc
```
...
```bash
# Start hashiumd in regtest mode
./build/bin/hashiumd -regtest -daemon

# Create a wallet
./build/bin/hashium-cli -regtest createwallet "dev"

# Get a new address
./build/bin/hashium-cli -regtest getnewaddress

# Mine 101 blocks (to have spendable coins)
./build/bin/hashium-cli -regtest generatetoaddress 101 "YOUR_ADDRESS"

# Check balance
./build/bin/hashium-cli -regtest getbalance

# Stop the node
./build/bin/hashium-cli -regtest stop
```
...
class MyNewTest(HashiumTestFramework):
...
ls /tmp/hashium_func_test_*/test_framework.log

# Combine logs for debugging
python3 test/functional/combine_logs.py /tmp/hashium_func_test_XXXXX/
```
