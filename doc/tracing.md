# User-space, Statically Defined Tracing (USDT) for Hashium Core

Hashium Core includes statically defined tracepoints to allow for more
...
**Note**: Hashium Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Hashium Core
...
Multiple tools can list the available tracepoints in a `hashiumd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Hashium Core, use `info probes` in `gdb`:

```
$ gdb ./build/bin/hashiumd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/hashiumd
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/hashiumd
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/hashiumd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Hashium Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/hashiumd | grep NT_STAPSDT -A 4 -B 2
