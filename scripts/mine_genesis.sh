...
echo "Building /tmp/genesis_hashium ..."
...
THREADS="${HSMIUM_GENESIS_THREADS:-$(sysctl -n hw.ncpu)}"
MAIN_START_OPT=""
if [[ -n "${HSMIUM_GENESIS_START:-}" ]]; then
  MAIN_START_OPT="HSMIUM_GENESIS_START=${HSMIUM_GENESIS_START}"
fi
...
run_net() {
  ...
  env HSMIUM_GENESIS_THREADS="${THREADS}" HSMIUM_GENESIS_NETS="${net}" ${extra_env} /tmp/genesis_hashium | tee "${log}"
...
# Order: testnet, test4, regtest, main (main can resume with HSMIUM_GENESIS_START)
run_net "testnet" ""
run_net "test4" ""
run_net "regtest" ""
