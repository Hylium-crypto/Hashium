...
const char * const HSMIUM_CONF_FILENAME = "hashium.conf";
const char * const HSMIUM_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //hashium-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", HSMIUM_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Hashium
    //   new: C:\Users\Username\AppData\Local\Hashium
    // macOS: ~/Library/Application Support/Hashium
    // Unix-like: ~/.hashium
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Hashium";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Hashium";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Hashium";
#else
    // Unix-like
    return pathRet / ".hashium";
#endif
#endif
}
