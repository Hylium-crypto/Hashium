...
static void SetupHashiumTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " hashium-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The hashium-tx tool is used for creating and modifying hashium transactions.\n\n"
                "hashium-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded hashium transaction, or with \"-create [commands]\" to create a hex-encoded hashium transaction.\n"
                "\n"
                "Usage: hashium-tx [options] <hex-tx> [commands]\n"
                "or:    hashium-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded hashium transaction
