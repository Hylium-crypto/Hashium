// Copyright (c) 2025 The Hashium Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries hashium-node, hashium-gui.
  -M, --monolithic       Run monolithic binaries hashiumd, hashium-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'hashium-qt [ARGS]' or 'hashium-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'hashiumd [ARGS]' or 'hashium-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'hashium-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'hashium-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'hashium-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_hashium [ARGS]'.
  chainstate [ARGS] Run hashium kernel chainstate util, equivalent to running 'hashium-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_hashium [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_hashium-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "hashium-gui" : "hashium-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "hashium-node" : "hashiumd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("hashium-cli");
            // Since "hashium rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("hashium-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("hashium-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_hashium");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("hashium-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_hashium");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_hashium-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("hashium-util");
        } else {
...
//! Execute the specified hashiumd, hashium-qt or other command line in `args`
...
//!             be a relative path to a hashiumd, hashium-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/hashium ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/hashium/hashium/pull/31375#discussion_r1861814807)
