...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking hashium code so they have to run
    ...
    // functions run, so client objects are fully disconnected before hashium
    // code in the destructors are run. This way if the hashium code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In hashium core, running these callbacks in the right order is
