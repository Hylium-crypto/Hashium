// Copyright (c) 2011-present The Hashium Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <qt/guiutil.h>

#include <qt/hashiumaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Hashium address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new HashiumAddressEntryValidator(parent));
    widget->setCheckValidator(new HashiumAddressCheckValidator(parent));
}
...
bool parseHashiumURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no hashium: URI
    if(!uri.isValid() || uri.scheme() != QString("hashium"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!HashiumUnits::parse(HashiumUnit::HSM, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseHashiumURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseHashiumURI(uriInstance, out);
}

QString formatHashiumURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("hashium:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(HashiumUnits::format(HashiumUnit::HSM, info.amount, false, HashiumUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openHashiumConf()
{
    ...
    /* Open hashium.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Hashium.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Hashium (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Hashium (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Hashium*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "hashium.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("hashium-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a hashium.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Hashium\n";
        else
            optionFile << strprintf("Name=Hashium (%s)\n", ChainTypeToString(chain));
