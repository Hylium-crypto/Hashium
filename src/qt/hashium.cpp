...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<HashiumUnit>("HashiumUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. hashium_de.qm (shortcut "de" needs to be defined in hashium.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. hashium_de_DE.qm (shortcut "de_DE" needs to be defined in hashium.qrc)
...
static const char* qt_argv = "hashium-qt";

HashiumApplication::HashiumApplication()
...
void HashiumApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the HashiumApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", HashiumGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void HashiumApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool HashiumApplication::createOptionsModel(bool resetSettings)
{
...
void HashiumApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new HashiumGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &HashiumGUI::quitRequested, this, &HashiumApplication::requestShutdown);
...
void HashiumApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void HashiumApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool HashiumApplication::baseInitialize()
{
    return node().baseInitialize();
}

void HashiumApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &HashiumApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &HashiumApplication::handleRunawayException);
    connect(this, &HashiumApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &HashiumApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void HashiumApplication::parameterSetup()
{
...
void HashiumApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void HashiumApplication::requestInitialize()
{
...
void HashiumApplication::requestShutdown()
{
...
void HashiumApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // hashium: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &HashiumGUI::handlePaymentRequest);
            connect(window, &HashiumGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void HashiumApplication::handleRunawayException(const QString &message)
{
...
void HashiumApplication::handleNonFatalException(const QString& message)
{
...
WId HashiumApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool HashiumApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", HashiumGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(hashium);
    Q_INIT_RESOURCE(hashium_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(HSMIUM_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see hashium-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see hashium-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse hashium.conf, determine network, switch to network specific
    ...
    // hashium: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
