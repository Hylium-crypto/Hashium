// Copyright (c) 2011-2022 The Hashium Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <hashium-build-config.h> // IWYU pragma: keep

#include <qt/hashiumgui.h>
...
const std::string HashiumGUI::DEFAULT_UIPLATFORM =
...
HashiumGUI::HashiumGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &HashiumGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &HashiumGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &HashiumGUI::showModalOverlay);
...
void HashiumGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Hashium address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and hashium: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &HashiumGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &HashiumGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &HashiumGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Hashium addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Hashium addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Hashium Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Hashium Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a hashium: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Hashium command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &HashiumGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &HashiumGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &HashiumGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &HashiumGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &HashiumGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &HashiumGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &HashiumGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &HashiumGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &HashiumGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &HashiumGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &HashiumGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &HashiumGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &HashiumGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &HashiumGUI::showDebugWindow);
}

void HashiumGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void HashiumGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &HashiumGUI::setCurrentWalletBySelectorIndex);
...
void HashiumGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &HashiumGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &HashiumGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &HashiumGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &HashiumGUI::showProgress);
...
#ifdef ENABLE_WALLET
void HashiumGUI::enableHistoryAction(bool privacy)
{
...
void HashiumGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &HashiumGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &HashiumGUI::removeWallet);
...
WalletController* HashiumGUI::getWalletController()
{
    return m_wallet_controller;
}

void HashiumGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &HashiumGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &HashiumGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &HashiumGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &HashiumGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &HashiumGUI::incomingTransaction);
    connect(this, &HashiumGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void HashiumGUI::removeWallet(WalletModel* walletModel)
{
...
void HashiumGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void HashiumGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void HashiumGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void HashiumGUI::setWalletActionsEnabled(bool enabled)
{
...
void HashiumGUI::createTrayIcon()
{
...
void HashiumGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &HashiumGUI::toggleHidden);
...
void HashiumGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void HashiumGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void HashiumGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void HashiumGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void HashiumGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void HashiumGUI::openClicked()
{
...
void HashiumGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void HashiumGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void HashiumGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void HashiumGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void HashiumGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void HashiumGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void HashiumGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void HashiumGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Hashium network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void HashiumGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void HashiumGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void HashiumGUI::updateHeadersSyncProgressLabel()
{
...
void HashiumGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void HashiumGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &HashiumGUI::quitRequested);
...
void HashiumGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void HashiumGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &HashiumGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void HashiumGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void HashiumGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &HashiumGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &HashiumGUI::show);
                e->ignore();
            }
        }
    }
...
void HashiumGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void HashiumGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void HashiumGUI::incomingTransaction(const QString& date, HashiumUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(HashiumUnits::formatWithUnit(unit, amount, true));
...
void HashiumGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void HashiumGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool HashiumGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool HashiumGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void HashiumGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void HashiumGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void HashiumGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void HashiumGUI::updateProxyIcon()
{
    std::string ip_port;
...
void HashiumGUI::updateWindowTitle()
{
...
void HashiumGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void HashiumGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void HashiumGUI::detectShutdown()
{
...
void HashiumGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void HashiumGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(HashiumGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = HashiumGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void HashiumGUI::subscribeToCoreSignals()
{
...
void HashiumGUI::unsubscribeFromCoreSignals()
{
...
bool HashiumGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<HashiumUnit> units = HashiumUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const HashiumUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, HashiumUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const HashiumUnit u : HashiumUnits::availableUnits()) {
        menu->addAction(HashiumUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(HashiumUnit newUnits)
{
    setText(HashiumUnits::longName(newUnits));
}
