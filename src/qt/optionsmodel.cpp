...
/** Call node.updateRwSetting() with Hashium 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Hashium 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/hashium/hashium/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because hashium 22.x will not try to read these.
...
//! Convert enabled/size values to hashium -prune setting.
...
//! Get pruning enabled value to show in GUI from hashium -prune setting.
...
//! Get pruning size value to show in GUI from hashium -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayHashiumUnit")) {
        settings.setValue("DisplayHashiumUnit", QVariant::fromValue(HashiumUnit::HSM));
    }
    QVariant unit = settings.value("DisplayHashiumUnit");
    if (unit.canConvert<HashiumUnit>()) {
        m_display_hashium_unit = unit.value<HashiumUnit>();
    } else {
        m_display_hashium_unit = HashiumUnit::HSM;
        settings.setValue("DisplayHashiumUnit", QVariant::fromValue(m_display_hashium_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if hashium.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_hashium_unit);
...
{
    if (new_unit.isNull() || new_unit.value<HashiumUnit>() == m_display_hashium_unit) return;
    m_display_hashium_unit = new_unit.value<HashiumUnit>();
    QSettings settings;
    settings.setValue("DisplayHashiumUnit", QVariant::fromValue(m_display_hashium_unit));
    Q_EMIT displayUnitChanged(m_display_hashium_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/hashium/hashium/pull/8273
    ...
    // (https://github.com/hashium-core/gui/issues/567).
