...
//! Specify whether parameter should be parsed by hashium-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make hashium-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `hashium-cli` attempt to parse the
 ...
 * `hashium-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make hashium-cli treat them as positional parameters when `-named` is used.
 * This prevents `hashium-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so hashium-cli
 ...
 *
 * For example, the command `hashium-cli -named createwallet "my=wallet"`,
