#!/usr/bin/env python3
# Copyright (c) 2020-present The Hashium Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""Test asmap config argument for ASN-based IP bucketing.

Verify node behaviour and debug log when launching hashiumd with different
...
class AsmapTest(HashiumTestFramework):
    ...
    def test_without_asmap_arg(self):
        self.log.info('Test hashiumd with no -asmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0)

    def test_noasmap_arg(self):
        self.log.info('Test hashiumd with -noasmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0, ["-noasmap"])

    def test_asmap_with_absolute_path(self):
        self.log.info('Test hashiumd -asmap=<absolute path>')
        self.stop_node(0)
    ...
    def test_asmap_with_relative_path(self):
        self.log.info('Test hashiumd -asmap=<relative path>')
        self.stop_node(0)
        name = 'ASN_map'
    ...
    def test_unspecified_asmap(self):
        ...
        for arg in ['-asmap', '-asmap=']:
            self.log.info(f'Test hashiumd {arg} (and no filename specified)')
            self.stop_node(0)
    ...
    def test_asmap_interaction_with_addrman_containing_entries(self):
        self.log.info("Test hashiumd -asmap restart with addrman containing new and tried entries")
        self.stop_node(0)
    ...
    def test_asmap_with_missing_file(self):
        self.log.info('Test hashiumd -asmap with missing map file')
        self.stop_node(0)
    ...
    def test_empty_asmap(self):
        self.log.info('Test hashiumd -asmap with empty map file')
        self.stop_node(0)
    ...
    def test_asmap_health_check(self):
        self.log.info('Test hashiumd -asmap logs ASMap Health Check with basic stats')
