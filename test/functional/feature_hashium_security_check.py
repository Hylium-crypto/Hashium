...
"""Security checks for Hashium network isolation and address validation.

This test verifies:
1. Hashium addresses are properly distinguished from Bitcoin addresses
...
class HashiumSecurityTest(HashiumTestFramework):
    ...
    def run_test(self):
        self.log.info("Running Hashium security tests...")
    ...
    def test_address_formats(self):
        """Test that Hashium uses unique address prefixes."""
        ...
        # Verify Hashium HRP prefix (regtest uses 'rhyl')
        ...
        assert not result['isvalid'], "Bitcoin address should be invalid on Hashium"
        ...
        assert not result['isvalid'], "Bitcoin mainnet address should be invalid on Hashium"
    ...
    def test_network_params(self):
        ...
        # Verify Hashium-specific subversion
        assert "Hashium" in info['subversion'] or "hashium" in info['subversion'].lower(), \
            f"Expected Hashium in subversion, got: {info['subversion']}"
...
if __name__ == "__main__":
    HashiumSecurityTest(__file__).main()
