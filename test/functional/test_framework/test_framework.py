#!/usr/bin/env python3
# Copyright (c) 2014-present The Hashium Core developers
...
TMPDIR_PREFIX = "hashium_func_test_"
...
class HashiumTestMetaClass(type):
    """Metaclass for HashiumTestFramework.

    Ensures that any attempt to register a subclass of `HashiumTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'HashiumTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("HashiumTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("HashiumTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class HashiumTestFramework(metaclass=HashiumTestMetaClass):
    """Base class for a hashium test script.

    Individual hashium test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave hashiumds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use hashium-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("HashiumRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a hashiumd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple hashiumd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the hashiumd
    ...
    def _start_logging(self):
        ...
        # Format logs the same as hashiumd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in hashium.conf
    ...
    def skip_if_no_hashiumd_tracepoints(self):
        """Skip the running test if hashiumd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("hashiumd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_hashiumd_zmq(self):
        """Skip the running test if hashiumd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("hashiumd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if hashium-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("hashium-wallet has not been compiled")

    def skip_if_no_hashium_tx(self):
        """Skip the running test if hashium-tx has not been compiled."""
        if not self.is_hashium_tx_compiled():
            raise SkipTest("hashium-tx has not been compiled")

    def skip_if_no_hashium_util(self):
        """Skip the running test if hashium-util has not been compiled."""
        if not self.is_hashium_util_compiled():
            raise SkipTest("hashium-util has not been compiled")

    def skip_if_no_hashium_chainstate(self):
        """Skip the running test if hashium-chainstate has not been compiled."""
        if not self.is_hashium_chainstate_compiled():
            raise SkipTest("hashium-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if hashium-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("hashium-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether hashium-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether hashium-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_hashium_tx_compiled(self):
        """Checks whether hashium-tx was compiled."""
        return self.config["components"].getboolean("BUILD_HSMIUM_TX")

    def is_hashium_util_compiled(self):
        """Checks whether hashium-util was compiled."""
        return self.config["components"].getboolean("ENABLE_HSMIUM_UTIL")

    def is_hashium_chainstate_compiled(self):
        """Checks whether hashium-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_HSMIUM_CHAINSTATE")
