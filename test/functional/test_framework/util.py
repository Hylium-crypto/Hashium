...
def assert_fee_amount(fee, tx_size, feerate_HSM_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_HSM_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s HSM too low! (Should be %s HSM)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_HSM_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s HSM too high! (Should be %s HSM)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting HSM values"""
...
class Binaries:
    """Helper class to provide information about hashium binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke hashiumd"
        return self._argv("node", self.paths.hashiumd, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke hashium-cli"
        # Add -nonamed because "hashium rpc" enables -named by default, but hashium-cli doesn't
        return self._argv("rpc", self.paths.hashiumcli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke hashium-tx"
        return self._argv("tx", self.paths.hashiumtx)

    def util_argv(self):
        "Return argv array that should be used to invoke hashium-util"
        return self._argv("util", self.paths.hashiumutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke hashium-wallet"
        return self._argv("wallet", self.paths.hashiumwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke hashium-chainstate"
        return self._argv("chainstate", self.paths.hashiumchainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the hashium wrapper executable (if HSMIUM_CMD is set or
        need_ipc is True), or the direct binary path (hashiumd, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.hashium_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the hashium
            # wrapper binary and append -m so it calls multiprocess binaries.
            hashium_cmd = self.paths.hashium_cmd or [self.paths.hashium_bin]
            return hashium_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]
...
def get_binary_paths(config):
    ...
    binaries = {
        "hashium": "HSMIUM_BIN",
        "hashiumd": "HSMIUMD",
        "hashium-cli": "HSMIUMCLI",
        "hashium-util": "HSMIUMUTIL",
        "hashium-tx": "HSMIUMTX",
        "hashium-chainstate": "HSMIUMCHAINSTATE",
        "hashium-wallet": "HSMIUMWALLET",
    }
    # Set paths to hashium core binaries allowing overrides with environment
    # variables.
    ...
    # HSMIUM_CMD environment variable can be specified to invoke hashium
    # wrapper binary instead of other executables.
    paths.hashium_cmd = shlex.split(os.getenv("HSMIUM_CMD", "")) or None
    return paths
...
def get_fee(tx_size, feerate_hyl_kvb):
    """Calculate the fee in HSM given a feerate is HSM/kvB. Reflects CFeeRate::GetFee"""
    ...
    return target_fee_sat / Decimal(1e8) # Return result in  HSM
...
def wait_until_helper_internal(predicate, *, timeout=60, lock=None, timeout_factor=1.0, check_interval=0.05):
    ...
    from `HashiumTestFramework` or `P2PInterface` class ensures the timeout is
...
def bpf_cflags():
    return [
        ...
        "-Wno-duplicate-decl-specifier",  # https://github.com/hashium/hashium/issues/32322
    ]
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "hashium.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> tuple[dict, pathlib.Path]:
    ...
    if platform.system() == "Windows":
        ...
        datadir = temp_dir / "Hashium"
    else:
        env = dict(HOME=str(temp_dir))
        if platform.system() == "Darwin":
            datadir = temp_dir / "Library/Application Support/Hashium"
        else:
            datadir = temp_dir / ".hashium"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "hashium.conf"), 'a') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "hashium.conf")):
        with open(os.path.join(datadir, "hashium.conf"), 'r') as f:
            for line in f:
