...
`test/functional/test_framework/test_framework.py:HashiumTestFramework.main`.
...
BASE_SCRIPTS = [
    'feature_hashium_reorg_basic.py',
    # Scripts that are run by default.
    # Longest test should go first, to favor running tests in parallel
    # vv Tests less than 5m vv
    'feature_fee_estimation.py',
    'feature_hashium_devnet_smoke.py',
    'feature_hashium_p2p_propagation.py',
    'feature_hashium_security_check.py',
    'feature_hashium_wallet_basic.py',
    'feature_hashium_mining_basic.py',
    'feature_taproot.py',
    'feature_block.py',
    ...
    'interface_hashium_cli.py',
    'feature_bind_extra.py',
    'mempool_resurrect.py',
    'wallet_txn_doublespend.py --mineblock',
    'tool_hashium_chainstate.py',
    'tool_wallet.py',
    'tool_utils.py',
...
def main():
    ...
    enable_hashiumd = config["components"].getboolean("ENABLE_HSMIUMD")

    if not enable_hashiumd:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if hashiumd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "hashiumd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a hashiumd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the Hashium functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/hashium/hashium/pull/27561.
...
class RPCCoverage():
    ...
    commands per `hashium-cli help` (`rpc_interface.txt`).
