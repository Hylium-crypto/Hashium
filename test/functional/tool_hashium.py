...
"""Test the hashium wrapper tool."""
from test_framework.test_framework import (
    HashiumTestFramework,
    SkipTest,
)
...
class ToolHashiumTest(HashiumTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `hashium node -version` is
        # run on windows, python doesn't capture output from the child
        # `hashiumd` and `hashium-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/hashium/hashium/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through hashium wrapper command with specified arguments."""
        node.args = [self.binary_paths.hashium_bin] + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure hashium node command invokes hashiumd by default")
        self.test_args([], [], expect_exe="hashiumd")

        self.log.info("Ensure hashium -M invokes hashiumd")
        self.test_args(["-M"], [], expect_exe="hashiumd")

        self.log.info("Ensure hashium -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure hashium -m invokes hashium-node")
            self.test_args(["-m"], [], expect_exe="hashium-node")

            self.log.info("Ensure hashium -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="hashium-node")

            self.log.info("Ensure hashium accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="hashium-node")

            self.log.info("Ensure hashium recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="hashium-node")
