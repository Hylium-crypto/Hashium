...
# Copyright 2016-present The Hashium Core developers
...
class ToolUtils(HashiumTestFramework):
    ...
    def skip_test_if_missing_module(self):
        self.skip_if_no_hashium_tx()
        self.skip_if_no_hashium_util()

    def run_test(self):
        self.testcase_dir = Path(self.config["environment"]["SRCDIR"]) / "test" / "functional" / "data" / "util"
        self.bins = self.get_binaries()
        with open(self.testcase_dir / "hashium-util-test.json") as f:
    ...
    def test_one(self, testObj):
        ...
        if testObj["exec"] == "./hashium-util":
            execrun = self.bins.util_argv() + testObj["args"]
        elif testObj["exec"] == "./hashium-tx":
