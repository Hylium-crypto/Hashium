...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from hashium-build-config.h is used without the header being included",
...
fn lint_std_filesystem() -> LintResult {
    let found = git()
        .args([
            ...
            ":(exclude)src/hashium-chainstate.cpp",
        ])
        .status()
        .expect("command error")
        .success();
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/hashium-build-config.h.in";
        ...
        git()
            .args([
                ...
                if mode {
                    ...
                    "#include <hashium-build-config.h>" // Catch redundant includes with and without the IWYU pragma
                },
                "--",
            ])
    ...
    if missing {
        return Err(format!(
            r#"
One or more files use a symbol declared in the hashium-build-config.h header. However, they are not
...
even though hashium-build-config.h indicates that a faster feature is available and should be used.
...
#include <hashium-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(r#"
None of the files use a symbol declared in the hashium-build-config.h header. However, they are including
